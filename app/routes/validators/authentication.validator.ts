...
export const loginSchema = Joi.object()
    .keys({
        identifier: Joi.alternatives(
            ...
            Joi.string().min(constants.USERNAME_MIN_LENGTH).max(constants.USERNAME_MAX_LENGTH).required()
        ).required(),
...
export const updatePasswordSchema = Joi.object().keys({
    oldPassword: Joi.string().min(constants.PASSWORD_MIN_LENGTH).max(constants.PASSWORD_MAX_LENGTH).required(),

    newPassword: Joi.string().min(constants.PASSWORD_MIN_LENGTH).max(constants.PASSWORD_MAX_LENGTH).required(),
});
