...
/**
 *  mustOwnUser ensures that the current authenticted is the same entity as the one the following
...
export const mustOwnUser = async (request: IRequest, response: Response, next: NextFunction) => {
    ...
    // Ensure that the requesting user is the entity they are also trying to perfrom the following
    ...
    if (request.user.id !== requestedUserId && request.user.role !== UserRole.ADMIN) {
        return response.sendStatus(401).json({
            message: 'Unauthorized, you can only perfrom this action on youserlf.',
        });
    }
